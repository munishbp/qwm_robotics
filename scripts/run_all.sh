...
run $P scripts/world_model_error.py --ckpt checkpoints/belief.pt > results/logs/wm_error.log 2>&1
for d in -1 0 2; do
  run $P scripts/evaluate.py --ckpt checkpoints/belief.pt --depth $d --lag 1 --out results/h1_depth$d.json > results/logs/h1_depth$d.log 2>&1
done
run $P scripts/sweep.py --ckpt checkpoints/belief.pt --which h2 h3 h4 robust transfer > results/logs/sweep.log 2>&1
run $P scripts/record_episodes.py --ckpt checkpoints/belief.pt   > results/logs/record.log 2>&1
