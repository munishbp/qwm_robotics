...
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"; cd "$ROOT"; P=.venv/bin/python
mkdir -p results/logs runs/mjlab/results/logs
echo "== $(date +%H:%M:%S) controls 2d"
SWARM_SIM=2d $P scripts/day1_controls.py --ckpt checkpoints/belief_best.pt --out results/day1_controls.json > results/logs/day1_2d.log 2>&1
echo "== $(date +%H:%M:%S) controls mjlab"
(cd runs/mjlab && SWARM_SIM=mjlab $P $ROOT/scripts/day1_controls.py --ckpt checkpoints/belief_best.pt --out results/day1_controls.json > results/logs/day1_mjlab.log 2>&1)
echo "== $(date +%H:%M:%S) demonstration quality control: collect at noise ${NOISE:-0.5}"
mkdir -p runs/demo65 && (cd runs/demo65 && SWARM_SIM=2d $P $ROOT/scripts/collect_offline.py --envs 256 --steps 800 --noise ${NOISE:-0.5} > collect.log 2>&1 && grep transitions collect.log)
echo "== $(date +%H:%M:%S) demonstration quality control: train"
(cd runs/demo65 && SWARM_SIM=2d $P $ROOT/scripts/train.py --obs belief --steps 12000 --out belief > train.log 2>&1)
